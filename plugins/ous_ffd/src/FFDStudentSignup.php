...
class FFDStudentSignup extends Page
{
    const DEFAULT_SLUG = "student_signup";
    ...
    {
        $result = DB::query()->from('ffd_student_rsvp')
            ->where('page_uuid = ?', [$this->uuid()])
            ->where('uuid = ?', [$uuid])
            ->fetch();
    ...
    public function newRSVP(string $for, string $firstName, string $lastName, string $shirtSize, ?int $guestCount, ?string $guestEmail, ?string $alumniParents, ?array $accommodations): string
    {
        $uuid = Digraph::uuid('rsvp');
        if (!$guestEmail) $guestEmail = null;
        if (!$alumniParents) $alumniParents = null;
        DB::query()->insertInto(
            'ffd_student_rsvp',
            [
                'uuid' => $uuid,
                ...
                'shirt' => htmlspecialchars($shirtSize),
                'guest_count' => intval($guestCount),
                'guest_email' => htmlspecialchars($guestEmail),
                'alumni_parents' => htmlspecialchars($alumniParents),
    ...
    public function updateRSVP(string $uuid, string $firstName, string $lastName, string $shirtSize, ?int $guestCount, ?string $guestEmail, ?string $alumniParents, ?array $accommodations)
    {
        if (!$guestEmail) $guestEmail = null;
        if (!$alumniParents) $alumniParents = null;
        DB::query()->update(
    ...
    {
        return DB::query()->from('ffd_student_rsvp')
            ->where('page_uuid = ?', [$this->uuid()])
            ->where('`for` = ?', [$netID])
            ->limit(1)
            ->fetch();
    ...
    {
        return ['ffd_student_signup', '_any'];
    }
}
