...
$query->order('institution.id DESC');
...
$table = new QueryTable(
            ...
            implode('', [
                (!$row['deprecated']
                    ? new ToolbarLink('Hide', 'hide', function () use ($id) {
                        Regalia::query()
                            ->update('institution', ['deprecated' => true], $id)
                            ->execute();
                    })
                    : new ToolbarLink('Show', 'show', function () use ($id) {
                        Regalia::query()
                            ->update('institution', ['deprecated' => false], $id)
                            ->execute();
                    }))
    ...
    [
        ...
        new ColumnHeader('Lining'),
        new ColumnHeader('Chevron'),
    ]
);
